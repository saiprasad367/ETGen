    ...
    try:
        ...
        if not hist.empty:
            ...
            if len(volume) >= 20:
                ...
                retail_pct = min(85, int(20 + vol_ratio * 15))
                if vol_ratio > 3.0:
                    bias_warnings.append(f"Herding bias: volume {vol_ratio:.1f}x average")
                    bias_ok = False
            ...
            if high_52w > 0 and current > high_52w * 0.97:
                bias_warnings.append(f"Near 52-week high (₹{high_52w:.0f}) — anchoring risk")
